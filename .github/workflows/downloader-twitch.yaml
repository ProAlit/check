...
on:
  workflow_dispatch:
    inputs:
      url:
        description: >
          Enter one or more Twitch VOD/Clip URLs.
    ...
    steps:
      ...
      - name: Get TwitchDownloader CLI
        ...
        run: |
          ...
          ASSET_NAME=$(echo "$RELEASE_JSON" | jq -r '.assets[].name' | grep -E '^TwitchDownloaderCLI-[0-9.]+-Linux-x64\.zip$' | head -1)
          if [ -z "$ASSET_NAME" ]; then
            echo "❌ Could not find Linux-x64 asset"
            exit 1
          fi
          DOWNLOAD_URL=$(echo "$RELEASE_JSON" | jq -r --arg name "$ASSET_NAME" '.assets[] | select(.name==$name) | .browser_download_url')
          echo "→ Downloading $ASSET_NAME ..."
          curl -sSL -o twitchdownloader.zip "$DOWNLOAD_URL"
          unzip -q twitchdownloader.zip -d twitch_cli
          chmod +x twitch_cli/TwitchDownloaderCLI
          sudo mv twitch_cli/TwitchDownloaderCLI /usr/local/bin/
          rm -rf twitch_cli twitchdownloader.zip
          echo "✅ TwitchDownloaderCLI ready"
      ...
      - name: Download all Twitch videos concurrently
        run: |
          set -e
          ...
          download_one() {
            ...
            local outfile="${OUTPUT_DIR}/twitch_${sanitized}.mp4"
            echo "→ Downloading: $url"
            QUALITY_ARG=""
            if [ -n "${{ inputs.quality }}" ]; then
              QUALITY_ARG="-q ${{ inputs.quality }}"
            fi
            if TwitchDownloaderCLI -m VideoDownload -u "$url" -o "$outfile" --ffmpeg-path /usr/bin/ffmpeg $QUALITY_ARG; then
        ...
        run: |
          ...
          if [ "$BUNDLE_ALL" = "true" ]; then
            ...
            if [ -n "$(ls -A "$BUNDLE_DIR" 2>/dev/null)" ]; then
              ...
              BUNDLE_ZIP="twitch_${RAND_BUNDLE}.zip"
      ...
      - name: Create Release and upload assets (release method)
        ...
        run: |
          ...
          TAG="twitch-downloader-${RAND5}"
          echo "Creating release tag: $TAG"
          git config user.name "GitHub Actions"
          git config user.email "actions@github.com"
          git tag "$TAG" -m "Twitch download release"
          git push origin "$TAG"
          ASSET_ARGS=""
          ...
          gh release create "$TAG" $ASSET_ARGS \
            --title "twitch-downloader-${RAND5}" \
            --notes "Automated release from Twitch Downloader workflow." \
            --target "${{ github.ref_name }}"

      - name: Cleanup
        if: always()
        run: |
          rm -rf twitch urls.txt failed_urls.txt twitch_assets.txt 2>/dev/null || true
          sudo wg-quick down wg0 2>/dev/null || true